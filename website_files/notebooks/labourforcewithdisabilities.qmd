...
To prepare the dataset, we first filter for `All disability types` in `disability_type` to prevent double-counting across specific impairments. We then separate the `Employment Rate` and `Total labour force` populations into distinct subsets.
...
Next, we merge the subsets back together to properly align each rate with its respective population weight, and convert the employment rate percentage into a 0-1 proportion to prepare it for our regression model. Because there is no missing data in this dataset, we do not need to worry about dropping NA values.
...
ggplot(labour_force_clean, aes(
  x = employment_rate, 
  y = factor(global_severity, levels = rev(sev_order)),
  fill = factor(global_severity, levels = sev_order)
)) +
...
sns.barplot(
    ...
    y="geography",
    hue="geography",
    order=geo_order_desc,
    hue_order=geo_order_asc,
    errorbar=None,
    palette="magma",
    legend=False,
    width=0.6,
...
We will use a Generalized Linear Model (GLM) with a `Binomial` family to handle fractional response variables (proportions between 0 and 1). By passing our population headcounts into the `weights` argument, the model evaluates the dataset as millions of surveyed individuals rather than treating each aggregated row equally. 
...
Next, we feed our prepared variables into the model. We will be using a GLM with a logit link to transform the data into log-odds. We are doing this because our outcome variable is a proportion bounded between 0 and 1; a standard linear model assumes an unbounded continuous outcome and could produce impossible predictions (like employment rates below 0% or above 100%). 

Furthermore, to accurately reflect the true scale of the aggregated data, we apply population weights. We also account for overdispersion—where the data has greater variance than a standard binomial model expects—by using the `quasibinomial` family in R and scaling by Pearson's chi-squared in Python.
