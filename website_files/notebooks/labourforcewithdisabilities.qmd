...
To prepare the dataset, we first filter for `All disability types` in `disability_type` to prevent double-counting across specific impairments. We then separate the employment rates and total labour force populations into distinct subsets.
...
**Note:** We use a Generalized Linear Model (GLM) with a `Binomial` family to handle fractional response variables (proportions between 0 and 1). By passing our population headcounts into the `weights` argument, the model evaluates the dataset as millions of surveyed individuals rather than treating each aggregated row equally. 
