...
We can see that teams with a lower seed tend to win more tournaments! We can also see the total amount of tourney wins for each seed.
...
## Python
```{python plot_data_py, warning = FALSE, message = FALSE}
...
Visually, the models don't appear to be significantly different from each other. If we wanted to formally evaluate which is the better approach we could use likelihood-based model selection tools.
...
## Python
```{python table_display_py, warning = FALSE}
...
## Python
```{python anova_result_py, warning=FALSE}
...
```{python overdispersion_py, warning=FALSE, message=FALSE}
...
```{python model_output_py, warning=FALSE}
...
1.  **Poisson regression supports seeding as a predictor**: The Poisson regression model indicates that seed is a significant predictor of tournament wins, suggesting that lower-seeded teams tend to win more, even between closely ranked seeds.
