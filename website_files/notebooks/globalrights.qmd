...
This case study explores patterns of missing data in a global LGBTQ+ rights dataset that includes economic and legal indicators such as GDP per capita, education spending. Using a variety of visualization tools, we aim to identify where and how missing values occur across different variables.
...
## Python
```{python load-libraries_py, message=FALSE, warning=FALSE, results=FALSE}
...
So far, we have gained a high-level overview of missingness across the dataset. To dig deeper, we now examine how missing values in one variable relate to the categories of another. This helps us understand whether missingness may be random or related to other features in the data.
...
::: {.panel-tabset}
## R
```{r spineplot-gdp-by-marriage, message = FALSE, fig.height=5, fig.width=6}
...
mtext("Missingness of GDP per Capita by Same-Sex Marriage Status", line = 1)
```
...
::: {.panel-tabset}
## R
```{r spineplot-edu-by-marriage, message = FALSE, fig.height=5, fig.width=6}
...
mtext("Same Sex Marriage Status", side = 1, line = 13) 
mtext("Proportion of Missing/Observed", side = 2, line = 3, adj = 1)
mtext("Missingness of Education Spending by Same-Sex Marriage Status", line = 1)
```
...
We used [MICE](https://www.jstatsoft.org/article/view/v045i03) with predictive mean matching (PMM) because it is well-suited to datasets with multiple partially missing variables, allows for realistic value imputation, and accommodates the MAR mechanism observed in our dataset. 
