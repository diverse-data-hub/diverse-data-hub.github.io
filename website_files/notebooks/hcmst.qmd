...
We won't forget to only `fit_transform` on the training set and only `transform` on the testing set! We don't want to break the golden rule.
...
Now that our data has been preprocessed, let's fit our model!

Since `scikit-learn` doesn't support ordinal regression at the time of writing, we will use a multinomial logistic regression model via `LogisticRegression()` which will automatically be multinomial if there is more than 2 target classes. We will set `class_weight='balanced'` to try to account for the class imbalance in the target variable and again set the random state for reproducibility.

```{python fit_model_py, include = FALSE}
logistic_regression_model = LogisticRegression(
    class_weight="balanced", random_state=123, max_iter=250, n_jobs=-1
)
...
abs_coef_df = pd.DataFrame(
    np.abs(logistic_regression_model.coef_.T), columns=classes, index=feature_names
)
...
For the "Good" class, the most important feature is "Same Sex Couple".  
...
Let's see how well our model predicts using unseen data. First, via a confusion matrix:
...
- Predictions for the true "Good" quality are concentrated at "Excellent".
- The model tends to predict higher relationship quality. This may be partly due to the small number of samples for the lower relationship quality classes, despite using balanced class weights.
...
From the above metrics, we can see that accuracy is low (0.40), but accuracy can be misleading with imbalanced data.  
However, the f1-scores (with an unweighted average of 0.22 and a weighted average of 0.44) still indicate that the model is not performing well, especially for the lower relationship quality classes.
...
```{python baseline_model_fit_py, include = FALSE}
baseline_model = DummyClassifier()
baseline_model.fit(X_train_transformed, y_train)
baseline_y_pred = baseline_model.predict(X_test_transformed)
```

```{python baseline_model_confusion_matrix_py}
cm = ConfusionMatrixDisplay.from_estimator(
    baseline_model,
    X_test_transformed,
    y_test,
...
```{python baseline_model_metrics_py, warning = FALSE}
print(
    classification_report(
        y_test,
        baseline_y_pred,
...
While the baseline model achieves a higher accuracy (0.50) compared to the logistic regression model (0.40), we cannot conclude that it performs better solely based on this metric, due to the class imbalance in the data.

In fact, the logistic regression model performs slightly better in terms of the unweighted F1-score average (0.22 vs. 0.13) and the weighted F1-score average (0.44 vs. 0.34), suggesting that it offers more balanced performance across all classes.
