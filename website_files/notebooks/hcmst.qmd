...
The logistic regression model has the best validation score and performs better than a baseline dummy model that only predicts the most common class in the training data. Let's tune it.
...
gs = GridSearchCV(
    lr_pipe, param_grid=param_grid, scoring="f1_weighted", n_jobs=-1, return_train_score=True
)
...
From the table above, we can see that setting `class_weight=None` and `C=500` yields the best mean validation score with little variation between the validation scores (`std_test_score = 0.024966`), indicating that the model has a stable performance and is not overly sensitive to which subset of the training data it sees.
...
abs_coef_df = pd.DataFrame(
    np.abs(gs.best_estimator_.named_steps['logisticregression'].coef_.T), columns=classes, index=feature_names
)
...
We will also see in the subsequent part of the analysis that the model does not have a very high performance and these specific feature importances may not be helpful in practical applications, but are kept in this analysis for demonstration purposes.
::::
