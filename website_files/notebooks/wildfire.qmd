...
If the goal is safety and preparedness, the model performance may be desirable. It might even be better to perform the hyperparameter search using recall or f-beta score with more weight on recall instead of f1 score.

If the goal is efficient use of resources, the model performance is not desirable due to many false positives. It would be better to perform the hyperparameter seach using precision or f-beta score with more weight on precision instead of f1 score.
